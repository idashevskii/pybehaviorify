...
class TestMemSequence(unittest.TestCase):
    ...
    async def test_initialization(self):
        node = pybehaviorify.MemSequence()
    ...
    async def test_success(self):
        ...
        mem_sequence = pybehaviorify.MemSequence(children=[node1, node2, node3])
        tick = TickStub()
        tick.blackboard.get.return_value = 0 # type: ignore
        status = mem_sequence.tick(tick)
    ...
    async def test_running(self):
        ...
        mem_sequence = pybehaviorify.MemSequence(children=[node1, node2, node3, node4])
        tick = TickStub()
        tick.blackboard.get.return_value = 0 # type: ignore
        status = mem_sequence.tick(tick)
    ...
    async def test_memory(self):
        ...
        node2 = get_node(NodeStatus.SUCCESS)
        node3 = get_node(NodeStatus.RUNNING)
        node4 = get_node(NodeStatus.FAILURE)
        node5 = get_node(NodeStatus.SUCCESS)

        mem_sequence = pybehaviorify.MemSequence(
            children=[node1, node2, node3, node4, node5]
        )
        mem_sequence.id = "node1"
        tick = TickStub()
        tick.blackboard.get.return_value = 0 # type: ignore
        status = await mem_sequence._execute(tick)
