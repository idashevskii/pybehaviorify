...
class TestPriority(unittest.TestCase):
    ...
    async def test_initialization(self):
        node = pybehaviorify.Selector()

        self.assertIsNotNone(node.id)
        self.assertEqual(node.name, 'Selector')
        self.assertEqual(node.title, 'Selector')
    ...
    async def test_success(self):
        ...
        sequence = pybehaviorify.Selector(children=[node1, node2, node3])
    ...
    async def test_running(self):
        ...
        sequence = pybehaviorify.Selector(children=[node1, node2, node3, node4])
        status = sequence.tick(TickStub())
